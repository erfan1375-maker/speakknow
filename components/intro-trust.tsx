...
export function IntroTrust() {
  return (
    <>
      {/* Desktop: a column of cards beside the photo, each tied to it with a dashed lead line */}
            ...
            <li key={item.title} className="flex items-center gap-0">
              ...
              >
                <div>
                  ...
                  <p className="mt-0.5 text-xs text-ink-faint">{item.body}</p>
                </div>
              ...
              <span
                aria-hidden
                className="animate-draw-line h-px w-8 shrink-0 border-t-2 border-dashed border-brand-300"
                style={{ animationDelay: `${delay}ms` }}
              />
              <span
                aria-hidden
